...
def calculate_psnr(syn_img, raw_img):
    psnr = peak_signal_noise_ratio(syn_img, raw_img, data_range=1.0)
    return psnr

def batch_PSNR(img, imclean, mask=None):
    Img = img.data.cpu().numpy().squeeze().clip(min=0., max=1.)
    Iclean = imclean.data.cpu().numpy().squeeze().clip(min=0., max=1.)
    if mask is not None:
        mask = mask.data.cpu().numpy().squeeze()
        mask[mask < 1] = 1
        PSNR = calculate_psnr(Iclean * mask, Img * mask)
    else:
        PSNR = calculate_psnr(Iclean, Img)
    return PSNR

def batch_L1(img, imclean, mask=None):
    Img = img.data.cpu().numpy().squeeze().clip(min=0., max=1.)
    Iclean = imclean.data.cpu().numpy().squeeze().clip(min=0., max=1.)
    if mask is not None:
        mask = mask.data.cpu().numpy().squeeze()
        mask[mask < 1] = 1
        L1 = np.abs(Img * mask - Iclean * mask).mean()
    else:
        L1 = np.abs(Img - Iclean).mean()
    return L1

def batch_SSIM(img, imclean, border=0):
    Img = img.data.cpu().numpy().squeeze().clip(0., 1.)
    Iclean = imclean.data.cpu().numpy().squeeze().clip(0., 1.)
    ...
    if Img.shape[0] == 3:
        ...
        Img_gray = 0.299 * R1 + 0.587 * G1 + 0.114 * B1
    else:
        Img_gray = Img[0]   # 单通道

    if Iclean.shape[0] == 3:
        R2, G2, B2 = Iclean[0], Iclean[1], Iclean[2]
        Iclean_gray = 0.299 * R2 + 0.587 * G2 + 0.114 * B2
    else:
        Iclean_gray = Iclean[0]

    SSIM = calculate_ssim(Iclean_gray, Img_gray)
    return SSIM
