...
from monai.losses import GlobalMutualInformationLoss as GMILoss
...
from .regist_net import Encoder, ModelTransfer_lite, RegNet_lite, r_loss, project, TRNet
...
class CrossAttention(nn.Module):
    ...
    def forward(self, f1, f2):
        ...
        att_map = th.reshape(att_map, [att_shape[0], att_shape[1], -1])
        att_map = F.softmax(att_map, dim=2)
        att_map = th.reshape(att_map, att_shape)
        f1 = f1 * att_map
        f1 = f1 + f1_hat
        return f1
...
class RegBlock(nn.Module):
    ...
    def forward(self, A, B): # f2是cat后的特征
        B = self.conv1x1(B)
        A = self.crossAtt1(A, B)
        A = self.feature_output(A)
        flow = self.flow_output(A)
        return flow

class TimeawareRegBlock(nn.Module):
    """
    A modified residual block that decouples input features into A (kept) and B (registered)
    via learned MLP separation, performs registration on B, then concatenates them back.
    """
    def __init__(
        self,
        channels,
        ...
        # --- 通道划分 ---
        self.B_channels = int(channels * split_ratio)
        self.A_channels = channels - self.B_channels

        # === 特征解耦 MLP (使用 1x1 卷积) ===
        self.feature_splitter = nn.Sequential(
            nn.Conv2d(channels, channels, kernel_size=1),
            nn.SiLU(),
            nn.Conv2d(channels, channels, kernel_size=1)
        )
        self.reg = RegBlock(self.A_channels, self.B_channels)
        if not self.last:
            self.up = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                     nn.Conv2d(in_channels=2, out_channels=2, kernel_size=1, padding=0, stride=1),
                                     nn.Tanh(),)

    def forward(self, x, emb, flow=0):
        decouple_feature = self.feature_splitter(x)
        A, B = th.split(decouple_feature, [self.A_channels, self.B_channels], dim=1)
        if flow is not None:
            A = warp2D()(A, flow)
        flow = self.reg(A, B)
        if not self.last:
...
def flow_integration_ir(flows):
    flow2, flow3, flow4, flow5, flow6 = flows
    up2 = nn.Upsample(scale_factor=16, mode='bilinear', align_corners=True)
    up3 = nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)
    up4 = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
    up5 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
    flow2 = up2(flow2) * 16
    flow3 = up3(flow3) * 8
    flow4 = up4(flow4) * 4
    flow5 = up5(flow5) * 2
...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        self.TRNet = TRNet(num_stages=6, emb_dim=time_embed_dim,
                           in_channels=self.in_channels)

    def forward(self, x, timesteps, q_sample=None, noise=None, A=None, B=None, A_warp=None,
                x_real=None, test=False, return_flow=False, F1=None):
        """
        Apply the model to an input batch.
        :param x_real: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :param A_B: an [N x C x ...] Tensor of low quality iamge.
        :return: an [N x C x ...] Tensor of outputs.
        """
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels)).type(self.dtype)
        flow = self.TRNet(th.cat([A_warp, x[:, :self.in_channels // 2]], dim=1),
                          th.cat([B, x[:, self.in_channels//2:]], dim=1),
                          emb)
        A_fake = warp2D()(A_warp, flow)
        A_fake_B = th.cat((A_fake, B), dim=1)
        if not test:
            reg_loss = F.mse_loss(A_fake, A)
            flow_loss = r_loss(flow) # optional

        if F1 is not None:
            x = th.cat([F1, A_fake_B], dim=1)
        else:
            x_t_fake = q_sample(B, A_fake, timesteps, noise=noise)
            x_t_inv_fake = q_sample(A_fake, B, timesteps, noise=noise)
            x = th.cat((x_t_fake, x_t_inv_fake), dim=1)
            x = th.cat([x, A_fake_B], dim=1)
        ...
        if test:
            if return_flow:
                return out, A_fake, flow
            return out, A_fake
        return out, reg_loss, flow_loss, A_fake
